...
import argparse, json, os, random, threading, time, ssl
...
def mk_client(client_id: str, username: Optional[str] = None) -> mqtt.Client:
    ...
    c.tls_set(ca_certs="certs/cacert.pem",
...
def device_thread(device_name: str, csv_path: str, broker: str, port: int,
    ...
    client_id = f"{ZONE}-{username}-replayer-tls"
    ...
    while not connected:
        try:
            ...
            client.loop_start()
            connected = True
            print(f"[{ZONE}:{device_name}] Connected to {broker}:{port} (TLS)")
        ...
        while True:
            ...
            if _is_publish(row, msg_col):
                ...
                try:
                    ...
                    print(f"[{ZONE}:{device_name}] Row {i+1}/{len(df)} → published (TLS): {payload}")
                except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(description="CSV Replayer (Production Zone) - TLS")
    ...
    print("CSV Replayer (Production Zone) Starting with TLS...")
    ...
    for name, fname, username in DEVICES:
        ...
        t.start()
        threads.append(t)
        print(f"Started {name} → topic factory/{TENANT}/{name}/telemetry (file: {fname}) (TLS)")
