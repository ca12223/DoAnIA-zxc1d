...
class TopicEnumerationAttackTLS:
    def __init__(self, broker_host="localhost", broker_port=8883,
        ...
        # Added TLS related params (ca_certs, client_cert, client_key, insecure)
    ...
    def _print_cert_status(self):
        # New helper to provide diagnostics before attack
        if self.ca_certs:
    ...
    def create_client(self, client_id, username=None, password=None):
        ...
        try:
            ...
            # Use provided CA file if present; otherwise create default context
            if self.ca_certs and os.path.exists(self.ca_certs):
                ...
                # Fallback to system trust store via SSLContext
            ...
            # If explicit insecure mode requested, tell paho to be insecure
            if self.insecure:
                try:
    ...
    def enumeration_worker(self, worker_id, topic_candidates, delay_ms=200, username=None, password=None):
        ...
        client_id = f"topic_enum_tls_{worker_id}"
    ...
    def launch_attack(self, num_workers=2, delay_ms=200, custom_topics=None, username=None, password=None):
        ...
        # show TLS/CA status before starting (new)
...
def main():
    ...
    # Added TLS/CA CLI flags (same pattern as wildcard/publish fixes)
