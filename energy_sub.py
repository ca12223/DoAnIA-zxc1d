...
def build_client_id(prefix="truongphong_energy"):
    return f"{prefix}_{uuid.uuid4().hex[:8]}"

def on_connect(client, userdata, flags, reason_code, properties=None):
    print("Connected:", reason_code)
    rc, mid = client.subscribe("factory/energy/+/telemetry", qos=1)
...
def main():
    ...
    ap.add_argument("--username", default="truongphong_energy")
    ap.add_argument("--password", default="123")
    ap.add_argument("--tls", action="store_true")
    ap.add_argument("--cafile", default="certs/ca-cert.pem")  
    ap.add_argument("--insecure", action="store_true")
    ap.add_argument("--client-id", default="truongphong_energy_sub")         
    args = ap.parse_args()
